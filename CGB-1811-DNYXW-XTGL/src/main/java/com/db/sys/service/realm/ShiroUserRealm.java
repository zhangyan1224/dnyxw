...
public class ShiroUserRealm extends AuthorizingRealm{
	...
	/**
	 * 基于此方法完成认证信息获取以及封装
	 * 系统底层会将认证数据传递认证管理器，由认证
	 * 管理器完成认证操作
	 */
	@Override
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
		//1.获取用户名(用户页面输入)
		UsernamePasswordToken upTokenToken=(UsernamePasswordToken) token;
		String username = upTokenToken.getUsername();
		//2.基于用户名查询用户信息
		...
		SimpleAuthenticationInfo info = 
				new SimpleAuthenticationInfo(user, //principald代表身份信息
				user.getPassword(), //hashedCredentials比较凭证--已加密的密码
				credentialsSalt, //credentialsSalt凭证盐值
				getName());//realm的名字ShiroUserRealm
		//6.返回封装结果
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		//2.基于用户id查询用户对应的角色信息(sys_user_roles)
