...
public class SysGoumaiServiceImpl implements SysGoumaiService {
	...
	@Override
	public PageObject<SysShangpin> findPageObjects(String goumai_name, Integer pageCurrent) {
		//1.验证参数有效性
		...
		//2.基于条件查询总记录数并进行相关判定
		  //2.1) 执行查询总记录数
		int rowCount = sysGoumaiDao.getRowCount(goumai_name);
		...
		//3.3)执行当前数据的查询操作
		List<SysShangpin> records = sysGoumaiDao.findPageObjects(goumai_name,startIndex, pageSize);
		//4.对分页信息以及当前页记录进行封装
		//封装两次结果并返回
		//4.1)构建PageObject对象
	...
	@Override
	public PageObject<SysShangpin> findPageObject(String youxi_name,String goumai_name,Integer pageCurrent) {
		...
		//2.基于条件查询总记录数并进行相关判定
		  //2.1) 执行查询总记录数
		int rowCount = sysGoumaiDao.getRowCount1(youxi_name,goumai_name);
		//2.2) 验证查询结果，假如结果为0不再执行如下操作,为0说明没查到
		if (rowCount==0) {
			throw new ServiceException("系统没有查到对应记录");
		}
		...
		int startIndex=(pageCurrent-1)*pageSize;//(当前页码-1)*页面大小
		//3.3)执行当前数据的查询操作
		List<SysShangpin> records = sysGoumaiDao.findPageObject(youxi_name,goumai_name,startIndex, pageSize);
		//4.对分页信息以及当前页记录进行封装
		//封装两次结果并返回
		//4.1)构建PageObject对象
		PageObject<SysShangpin> pageObject = new PageObject<>();
