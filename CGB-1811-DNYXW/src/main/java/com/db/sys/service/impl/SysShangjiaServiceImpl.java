...
public class SysShangjiaServiceImpl implements SysShangjiaService {
	/**
	 * @Autowired 注解用于告诉spring框架请按类型从spring容器中查找对象,然后赋值给属性
	 * (底层通过反射实现),这个过程称为DI
	 */
	@Autowired
	@Qualifier("sysShangjiaDao")//可以基于此注解指定注入哪个实现类
	private SysShangjiaDao sysShangjiaDao;
	...
	public PageObject<SysShangpin> findPageObjects(String shangjia_name,Integer pageCurrent) {
		...
		//2.基于条件查询总记录数并进行相关判定
		  //2.1) 执行查询总记录数
		int rowCount = sysShangjiaDao.getRowCount(shangjia_name);
		...
		//3.3)执行当前数据的查询操作
		List<SysShangpin> records = sysShangjiaDao.findPageObjects(shangjia_name,startIndex, pageSize);
		//4.对分页信息以及当前页记录进行封装
		//封装两次结果并返回
		//4.1)构建PageObject对象
	...
	public int deleteObjcts(Integer... ids) {
		...
		int rows;
		try {
			rows = sysShangjiaDao.deleteObjcts(ids);
		} catch (Throwable e) {
			//3.发出报警信息(例如给运维人员发短信)
			e.printStackTrace();
	...
	public PageObject<SysShangpin> findPageObject(String youxi_name,String shangjia_name,Integer pageCurrent) {
		...
		//2.基于条件查询总记录数并进行相关判定
		  //2.1) 执行查询总记录数
		int rowCount = sysShangjiaDao.getRowCount1(youxi_name,shangjia_name);
		//2.2) 验证查询结果，假如结果为0不再执行如下操作,为0说明没查到
		if (rowCount==0) {
			throw new ServiceException("系统没有查到对应记录");
		}
		...
		int startIndex=(pageCurrent-1)*pageSize;//(当前页码-1)*页面大小
		//3.3)执行当前数据的查询操作
		List<SysShangpin> records = sysShangjiaDao.findPageObject(youxi_name,shangjia_name,startIndex, pageSize);
		//4.对分页信息以及当前页记录进行封装
		//封装两次结果并返回
		//4.1)构建PageObject对象
		PageObject<SysShangpin> pageObject = new PageObject<>();
