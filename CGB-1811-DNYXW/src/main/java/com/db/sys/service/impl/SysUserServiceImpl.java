...
public class SysUserServiceImpl implements SysUserService {
	...
	@Override
	public int updateMoney(Integer money) {
		if (money<=0) throw new ServiceException("请输入正确的数值");
		SysUser user = (SysUser) SecurityUtils.getSubject().getPrincipal();
		String username = user.getUsername();
		SysUser userByUserName = sysUserDao.findUserByUserName(username);
		Integer oldMoney = userByUserName.getMoney();
		Integer newMoney = oldMoney+money;
		System.out.println("SysUserServiceImpl:updateMoney:oldMoney="+oldMoney);
		System.out.println("SysUserServiceImpl:updateMoney:money="+money);
		System.out.println("SysUserServiceImpl:updateMoney:username="+username);
		int i = sysUserDao.updateMoney(username,newMoney);
		return i;
	}
