...
public class SysXiajiaController {
	...
	public JsonResult doFindPageObjects(Integer pageCurrent) {
		...
		PageObject<SysShangpin> PageObject = sysXiajiaService.findPageObjects(shangjia_name,pageCurrent);
	...
	public JsonResult doFindPageObject(String youxi_name,Integer pageCurrent) {
		SysUser user = (SysUser) SecurityUtils.getSubject().getPrincipal();
		String shangjia_name = user.getUsername();
		PageObject<SysShangpin> PageObject = sysXiajiaService.findPageObject(youxi_name,shangjia_name,pageCurrent);
